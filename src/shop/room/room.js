...
Page({
    ...
    onLoad(options) {
        ...
        const snews = wx.getStorageSync('s_news');
        if (snews) {
            if (snews[urlid]) {
                console.log('读取缓存数据开始');
                this.setData({
                    message: snews[urlid].message,
                    user_name: snews[urlid].user_name,
                    user_photo: snews[urlid].user_photo,
                    user_id: snews[urlid].user_id,
                    userinfo_url: snews[urlid].userinfo_url,
                    shop: snews[urlid].shop,
                    newsdata: snews,
                    getNewMsg: true,
                });
        ...
        function getMsgRecord() {
            ...
            wx.request({
                url: `${app.globalData.domain}/smallapp/im-shop/msg-record`,
                data: { id: options.id },
                header: {
                ...
                success(res) {
                    ...
                    if (res.data.code === 1) {
                        _self.setData({
                            ...
                            user_photo: options.photo,
                            user_id: options.id,
                            userinfo_url: `/shop/userinfo/userinfo?id=${options.id}`,
    ...
    onUnload() {
        ...
        console.log('页面关闭le ....');
        const _self = this;
        const optionsData = _self.data.first_options;
                ...
                success(res) {
                    ...
                    if (res.data.code === 1) {
                        ...
                        obj[optionsData.id] = {
                            ...
                            userinfo_url: `/user/shopinfo/shopinfo?id=${optionsData.id}`,
                            shop: wx.getStorageSync('me'),
                        };
                        // 管理缓存
    ...
    send(e) {
        ...
        function sendReq() {
            ...
            wx.request({
                url: `${app.globalData.domain}/smallapp/im-shop/send`,
                data: `id=${_self.data.user_id}&content=${content}&type=1&from=2`,
                method: 'POST',
                header: {
                    'access-token': wx.getStorageSync('token'),
        ...
        function sendFile() {
            ...
            wx.uploadFile({
                ...
                formData: {
                    ...
                    type: 3,
                    from: 2,
                    remark: parseInt(_self.data.voice_time / 1000),
                    content: 'voice',
                },
                header: {
                    'access-token': wx.getStorageSync('token'),
    ...
    messageFormat(message) {
        ...
        let preTime = this.data.last_msg_time;
        let flag = 0;
        Object.keys(message).forEach(i => {
            ...
            if (message[i].send_time - preTime > 600) {
                ...
                preTime = message[i].send_time;
            }
        });
        this.setData({
            last_msg_time: preTime,
        });
        return message;
    },
    msgRecord() {
    ...
    newMsg() {
        ...
        const uid = _self.data.user_id;
        if (_self.data.getNewMsg) {
            wx.request({
                url: `${app.globalData.domain}/smallapp/im-shop/msg`,
                data: { id: uid },
                header: {
                    'access-token': wx.getStorageSync('token'),
                },
