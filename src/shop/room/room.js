...
Page({
  ...
  onLoad:function(options){
    ...
    var s_news = wx.getStorageSync('s_news');
    if(s_news){
      if(s_news[url_id]){
        console.log("读取缓存数据开始")
        this.setData({
          message:s_news[url_id].message,
          user_name:s_news[url_id].user_name,
          user_photo:s_news[url_id].user_photo,
          user_id:s_news[url_id].user_id,
          userinfo_url:s_news[url_id].userinfo_url,
          shop:s_news[url_id].shop,
          newsdata:s_news,
          getNewMsg:true
        });
    ...
    function getMsgRecord(){
      ...
      wx.request({
        url: getApp().globalData.domain+'/smallapp/im-shop/msg-record', 
        data:{id:options.id},
        header: {
  ...
  send:function(e) {
    if(e.detail.value != null && e.detail.value.trim() !== ''){
      ...
      function sendReq(){
        ...
        wx.request({
          url: getApp().globalData.domain+'/smallapp/im-shop/send', 
      ...
      success: function (res) {
        ...
        function sendFile(){
          ...
          wx.uploadFile({
            ...
            formData:{id:_self.data.user_id,type:2,from:2,content:'img'},
            header: {
                'access-token': wx.getStorageSync('token'),
      ...
      success: function(res) {
        ...
        function sendFile(){
          ...
          wx.uploadFile({
            ...
            formData:{id:_self.data.user_id,type:3,from:2,remark:parseInt(_self.data.voice_time/1000),content:'voice'},
            header: {
                'access-token': wx.getStorageSync('token'),
                'Content-Type': 'application/json'
            },
  ...
  newMsg:function(){
    ...
    if(_self.data.getNewMsg){
      wx.request({
          url: getApp().globalData.domain+'/smallapp/im-shop/msg', 
          data:{id:uid},
          header: {
              'access-token': wx.getStorageSync('token')
          },
