...
Page({
   ...
   */
  onLoad (o) {
    let city = o.city, listName = o.listName, tagCookie = o.tagCookie,title=o.title || '列表页';
    console.log(o)
  ...
  getListData () {
    ...
    wx.request({
      ...
      success(response) {
        ...
        let listNewArr = that.data.listArr.concat(response.data.infos);
        that.setData({
          listArr: listNewArr
        });
        if (!app.globalData.listCookie){
          let _setcookie = response.header["Set-Cookie"] || response.header["set-cookie"] ;
  ...
  onShareAppMessage(){
    return {
      title: '58同城生活助手',
      desc: '列表页',
      path: 'discovery/liebiaoye/liebiaoye?city=' + this.data.city + '&listName=' + this.data.listName + '&title=' + this.data.title
    }
  }
